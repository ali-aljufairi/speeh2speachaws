...
let currentVideo = 'default.mp4';

// Function to find the best matching video for a book title
function findMatchingVideo(bookTitle) {
    // Normalize the book title for matching
    const normalizedTitle = bookTitle.toLowerCase()
        .replace(/[^a-z0-9]/g, '')
        .trim();
    
    // Get all video files from the videos directory
    return fetch('/videos/list')
        .then(response => response.json())
        .then(videos => {
            // Find the best matching video
            const bestMatch = videos.find(video => {
                const videoName = video.toLowerCase()
                    .replace(/[^a-z0-9]/g, '')
                    .trim();
                return videoName.includes(normalizedTitle) || 
                       normalizedTitle.includes(videoName);
            });
            
            return bestMatch || 'default.mp4';
        })
        .catch(error => {
...
async function changeBackgroundVideo(bookTitle) {
    const newVideo = await findMatchingVideo(bookTitle);
    if (newVideo !== currentVideo) {
        currentVideo = newVideo;
        backgroundVideo.src = `/videos/${newVideo}`;
...
// Modify the handleTextOutput function to detect book titles
function handleTextOutput(data) {
    ...
    if (data.content) {
        ...
        // If this is an assistant message, try to find a matching video
        if (data.role === 'ASSISTANT') {
            // Try to extract quoted book title first
            let bookTitle = null;
            const quotedMatch = data.content.match(/"([^"]+)"|'([^']+)'/);
            if (quotedMatch) {
                bookTitle = quotedMatch[1] || quotedMatch[2];
            } else {
                // Fallback: try to extract the first capitalized phrase (likely a book title)
                const capitalizedMatch = data.content.match(/([A-Z][a-z]+(?: [A-Z][a-z]+)+)/);
                if (capitalizedMatch) {
                    bookTitle = capitalizedMatch[1];
                }
            }
            if (bookTitle) {
                console.log('Detected book title for video:', bookTitle);
                changeBackgroundVideo(bookTitle);
            } else {
                console.log('No book title detected in assistant message.');
            }
        }
    }
}
